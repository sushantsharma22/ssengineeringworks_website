            ...
            <div className="container mx-auto px-6 lg:px-12 relative z-10">
                <div className="text-center max-w-3xl mx-auto mb-16">
                    ...
                    <h2 className="text-4xl md:text-5xl font-bold mb-6 font-heading tracking-tight" style={{ color: 'var(--dynamic-text-primary)' }}>
                        Comprehensive <span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-400 to-cyan-400">Pumping Solutions</span>
                    </h2>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 services-grid">
                    {services.map((service, index) => (
                        <Link
                            key={index}
                            href={service.link}
                            className="service-card group relative bg-slate-900/50 backdrop-blur-sm p-8 rounded-2xl border border-slate-800 hover:border-blue-500/50 transition-all duration-300 hover:-translate-y-1 hover:shadow-2xl hover:shadow-blue-500/10"
                        >
                            {/* Hover Gradient */}
                            ...
                            <div className="relative z-10">
                                <div className="w-14 h-14 bg-slate-800 rounded-xl flex items-center justify-center text-blue-400 mb-6 group-hover:bg-blue-600 group-hover:text-white transition-all duration-300 shadow-lg shadow-black/20 group-hover:shadow-blue-500/25">
                                    <service.icon size={28} />
                                </div>
                                <h3 className="text-xl font-bold mb-3 group-hover:text-blue-300 transition-colors" style={{ color: 'var(--dynamic-text-primary)' }}>
                                    {service.title}
                                </h3>
                                <p className="text-sm leading-relaxed mb-6 group-hover:text-slate-300 transition-colors" style={{ color: 'var(--dynamic-text-secondary)' }}>
                                    {service.description}
                                </p>
                                <span className="text-blue-400 text-sm font-semibold flex items-center gap-2 group-hover:gap-3 transition-all">
